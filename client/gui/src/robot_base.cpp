...
robot_base::robot_base(QObject *parent) :
    QObject(parent)
{
}
...
void robot_base::expand(QString &expr) {
    ...
    static QRegularExpression param_replace("\\$\\{([a-z0-9-_]+)\\}", QRegularExpression::CaseInsensitiveOption);
    ...
    while(i.hasNext()) {
        ...
        QString r;
        if(parameters.contains(m.captured(1))) {
            r = parameters[m.captured(1)];
        }
